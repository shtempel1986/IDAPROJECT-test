(function ($) {
	...
	$(()=>{
		...
		let $cvvInput = $('.payment-cvv input').on('keypress',function (){
			if($(this).val().length>1){
				$cardHolderInput.focus();
			}
		});

		let $cardHolderInput = $('.payment-form__cardholder input');
		...
		let $cardInputs = $('.payment-form__card input').each(function (idx) {
